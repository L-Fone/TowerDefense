	...
	private void OnGUI()
	{
		try
		{
			...
			if (GUILayout.Button("导出客户端配置"))
			{
				...
				ExportAllClass(@"./Assets/Model/Module/Demo/Config", "namespace ET\n{\n");
				ExportAllClass(@"./Assets/Hotfix/Module/Demo/Config", "using ET;\n\nnamespace ETHotfix\n{\n");
			...
			if (GUILayout.Button("导出服务端配置"))
			{
				...
				ExportAllClass(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "namespace ET\n{\n");
