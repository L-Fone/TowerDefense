...
public class ReferenceCollectorEditor: Editor
{
    ...
    private void AddReference(SerializedProperty dataProperty, string key, Object obj)
	{
		...
		if (obj is Texture2D)
		{
			Object[] sprites = AssetDatabase.LoadAllAssetsAtPath(AssetDatabase.GetAssetPath(obj));
			for (int i = 0; i < sprites.Length; i++)
			{
				if (i == 0) continue;

				index = dataProperty.arraySize;
				dataProperty.InsertArrayElementAtIndex(index);
				element = dataProperty.GetArrayElementAtIndex(index);
				element.FindPropertyRelative("key").stringValue = sprites[i].name;
				element.FindPropertyRelative("gameObject").objectReferenceValue = sprites[i];
			}
		}
		else
		{
