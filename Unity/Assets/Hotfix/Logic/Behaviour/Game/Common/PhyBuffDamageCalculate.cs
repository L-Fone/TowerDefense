...
namespace ET
{
    public static class PhyBuffDamageCalculate
    {
        ...
        public static void Calculate(NumericComponent num, NumericComponent numTarget,ValueCalculate valueCalculate_Self, ValueCalculate valueCalculate_Target,int skillId, out BallisticData data )
        {
            ...
            //var def = numTarget.GetAsInt(NumericType.PhyDef) + 0.01f;
            ...
            //    FP cri = 0.05f + 0.6f * (crirBase * crirBase / (crirBase + numTarget.GetAsFloat(NumericType.Rpcrir) + 0.01f));
            ...
            //FP criHurt = data.isCrit ? (( criBase* 2 + BaseCritDamage) / (criBase * 0.6f + numTarget.GetAsFloat(NumericType.Rpcri) + 1)) : 1;

            //data.value = baseHurt * criHurt * RandomHelper.RandomFloat(0.9f, 1.1f) * (1 - numTarget.GetAsFloat(NumericType.Nphyi));
        }
    }
}
