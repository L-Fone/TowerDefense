...
namespace ET
{
	public sealed class EventSystem: IDisposable
	{
		...
		private readonly Dictionary<string, List<IOldEvent>> allStrEvents = new Dictionary<string, List<IOldEvent>>();
		...
		public void Add(Assembly assembly)
		{
            ...
            this.allStrEvents.Clear();
           foreach (Type type in types[typeof(OldEventAttribute)])
            {
                object[] attrs = type.GetCustomAttributes(typeof(OldEventAttribute), false);

                foreach (object attr in attrs)
                {
                    OldEventAttribute aEventAttribute = (OldEventAttribute)attr;
                    object obj = Activator.CreateInstance(type);
                    IOldEvent iEvent = obj as IOldEvent;
                    if (iEvent == null)
                    {
        ...
        public void RegisterEvent(string eventId, IOldEvent e)
        {
            if (!this.allStrEvents.ContainsKey(eventId))
            {
                this.allStrEvents.Add(eventId, new List<IOldEvent>());
            }
            this.allStrEvents[eventId].Add(e);
        }
        ...
        {
            List<IOldEvent> iEvents;
            if (!this.allStrEvents.TryGetValue(type, out iEvents))
            {
                return;
            }
            foreach (IOldEvent iEvent in iEvents)
            {
                try
                {
                    iEvent?.Handle();
