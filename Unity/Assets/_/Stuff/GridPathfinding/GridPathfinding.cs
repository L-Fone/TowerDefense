    ...
    public class GridPathfinding {
        ...
        public void Raycast3DWalkable()
        {
            for (int i = 0; i < widthMax; i++)
            {
                ...
                {
                    Vector3 nodeWorldPosition = mapNodes[i][j].GetWorldVector(worldOrigin, nodeSize);
                    bool isCollider = Physics.CheckSphere(new Vector3(nodeWorldPosition.x,0,nodeWorldPosition.y),0.35f);
                    if (isCollider)
                    {
                        mapNodes[i][j].SetWalkable(false);
                    }
        ...
        public void PrintMap(Transform prefabWalkable, Transform prefabUnwalkable) {
            for (int x = 0; x < mapNodes.Length; x++) {
                for (int y = 0; y < mapNodes[x].Length; y++) {
                    ...
                    UnityEngine.Object.Instantiate(pathNode.IsWalkable() ? prefabWalkable : prefabUnwalkable, new Vector3(worldOrigin.x + x * nodeSize ,0, worldOrigin.y + y * nodeSize ), Quaternion.identity);
                }
            }
        }
