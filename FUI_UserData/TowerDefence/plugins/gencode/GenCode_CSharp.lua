local function genCode(handler)
    ...
    for i=0,classCnt-1 do
        ...
        writer:writeln()
        writer:writeln('namespace ET') --namespaceName
        writer:startBlock()
        -- 1
        writer:writeln([[[ObjectSystem]
    public class %s_AwakeSystem : AwakeSystem<%s, GObject>
    {
    ...
    {	
        ...
        /// </summary>
        public %s self;
            ]],classInfo.className,namespaceName,classInfo.resName,classInfo.superClassName)
        ...
        -- writer:writeln('public static GObject CreateGObject()')
        -- writer:startBlock()
        -- writer:writeln('return UIPackage.CreateObject("%s", "%s");',  handler.pkg.name, classInfo.resName)
        -- writer:endBlock()
        -- writer:writeln()
            ...
            writer:writeln([[public void Awake(GObject go)
    {
        ...
        -- 销毁
        if handler.project.type==ProjectType.MonoGame then
            writer:writeln("protected override void OnConstruct1123()")
            writer:startBlock()
        else
